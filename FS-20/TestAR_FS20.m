...
addpath('./DLSR-FS');
fea = fea/255;
lambda = [0.01];% the parameter of feature selection
...
for i=1:20%20 trials
    ...
    for j = 1:fold%1
        ...
        for k = 1:length_lamada%1
            ...
            [W, b] = FSRobust_ALM(temp_Train, temp_gnd_Train, 80, 0.01, 1.02, 1000)
    ...
    for n=1:length_d
        ...
        [W, b] = FSRobust_ALM(fea_Train, gnd_Train, 80, 0.1, 1.02, 1000)
        WW = W .^ 2;
        W_weight = sum(WW, 2); % sum the element row-by-row
        [Weight, index_sorted_features] = sort(-W_weight); %  sort them from the largest to the smallest
        % output the features
