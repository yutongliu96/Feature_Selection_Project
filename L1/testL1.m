...
load (filename);
fea = fea/255;
lambda = [0.001 0.0015 0.002 0.0025];% the parameter of feature selection
...
for i=1:20%20 trials
    ...
    for j = 1:fold%1
        ...
        for k = 1:length_lamada%1
            ...
            f=thresh(temp_Train, temp_gnd_Train, lambda(k));
            %   The second step:  Select the features and output the selected
            %   features
            [Weight, index_sorted_features] = sort(-abs(f)); %  sort them from the largest to the smallest
            % output the features
            for n=1:length_d
    ...
    for n=1:length_d
        ...
        f=thresh(fea_Train, gnd_Train, lambda(kk));
