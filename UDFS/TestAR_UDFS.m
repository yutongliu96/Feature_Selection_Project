...
% fea=im2double(fea);
fea = fea/255;
lambda = [1e-9 1e-6 1e-3 1 1e3 1e6 1e9];% the parameter of feature selection
...
for i=1:20%20 trials
    ...
    for j = 1:fold%1
        ...
        for k = 1:length_lamada%1
            ...
            M = LocalDisAna(temp_Train, para);
            size_M = size(M, 1)
            size(temp_Train)
            [W, b] = LquadR21_reg(M, 80, lambda(k), eye(size_M));

%            [W, b] = train_feature_selection(temp_Train, temp_gnd_Train, lambda(k), u_para, iters, epsilon);
    ...
    for n=1:length_d
        ...
        para.k = 5;
        para.lambda = 1000;
        M = LocalDisAna(fea_Train, para);
        size_M = size(M, 1)
        [W, b] = LquadR21_reg(M, 80, lambda(kk), eye(size_M));
%        [W, b] = train_feature_selection(fea_Train, gnd_Train, lambda(kk), u_para, iters, epsilon);
        WW = W .^ 2;
        W_weight = sum(WW, 2); % sum the element row-by-row
        [Weight, index_sorted_features] = sort(-W_weight); %  sort them from the largest to the smallest
        % output the features
