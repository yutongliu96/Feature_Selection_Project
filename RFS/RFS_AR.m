...
addpath('./RFS');
fea = fea/255;
lambda = [0.001 0.01 0.1 1 10 100 1000 10000 100000];% the parameter of feature selection
...
for i=1:20%20 trials
    ...
    for j = 1:fold%1
        ...
        for k = 1:length_lamada%1
            ...
            [W, b] = train_feature_selection(temp_Train, temp_gnd_Train, lambda(k), iters, epsilon);
    ...
    for n=1:length_d
        ...
        [W, b] = train_feature_selection(fea_Train, gnd_Train, lambda(kk), iters, epsilon);
        WW = W .^ 2;
        W_weight = sum(WW, 2); % sum the element row-by-row
        [Weight, index_sorted_features] = sort(-W_weight); %  sort them from the largest to the smallest
        % output the features
